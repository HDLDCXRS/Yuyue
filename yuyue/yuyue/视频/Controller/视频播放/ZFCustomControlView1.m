...
- (UIButton *)popBtn
{
    if (!_popBtn) {
        ...
        [_popBtn setImage:[UIImage imageNamed:@"白色箭头"] forState:UIControlStateNormal];
    }
    return _popBtn;
}
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.popBtn addTarget:self action:@selector(popClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)layoutSubviews {
    ...
    CGSize size = self.popBtn.currentImage.size;
    min_x = (iPhoneX && self.player.isFullScreen) ? 44: 15;
    min_y = 0;
    min_w = size.width;
    min_h = size.height;
    self.popBtn.frame = CGRectMake(min_x, min_y, min_w, min_h);
    self.popBtn.zf_centerY =self.topToolView.zf_centerY + 10;
    
    min_x = self.popBtn.zf_centerX +10;
    min_y = 0;
    min_w = min_view_w - min_x - 15;
    min_h = 30;
    self.titleLabel.frame = CGRectMake(min_x, min_y, min_w, min_h);
    self.titleLabel.zf_centerY = self.popBtn.zf_centerY;
    ...
    self.fullScreenBtn.zf_centerY = self.bottomToolView.zf_centerY;
